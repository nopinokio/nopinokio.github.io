...
(function ($, CONFIG) {
    ...
    function searchResultToDOM (searchResult) {
        ...
        if(!$container.children().length == 0){
            $wrapper.removeClass('no_data');
        } else {
            $wrapper.addClass('no_data');
        }


       
    }
    function resultBoxSize(){
        var $winWidth = $(window).width(),
            $winHeight = $(window).height();
        var insSection = '.box_result',
            searchBox = '.search_block';

        var $SBoxHeight = ($(searchBox).position().top) + ($(insSection).position().top),
            $maxHeight = $winHeight - $SBoxHeight;
    ...
    $.getJSON(CONFIG.CONTENT_URL, function (json) {
        ...
        $input.on('input', function () {
            ...
            if(keywords.length > 0){
                $('.wrap_search').addClass('keyword_on');
            } else {
    ...
    $(document).on('click focus', '.search-form-input', function () {
        ...
        if (!$('body').hasClass('is_search_open')) return;
        switch (e.keyCode) {
            case 27: // ESC
                $input.val('');
                $('.wrap_search').removeClass('keyword_on');
                $('body').removeClass('is_search_open'); break;
            case 38: // UP
